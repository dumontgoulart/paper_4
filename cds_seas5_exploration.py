...
def storm_tracker_mslp_updated(ds, df_ws, smooth_step=None, grid_conformity=False, large_box=6, small_box=3):
    ...
    da_eye_track_ini = ds.sel(time = start_date, lat = lat_ini, lon = lon_ini, method = 'nearest')
    ...
    for time_step in ds.time.values:
        ...
        if time_step == start_date:
            ...
            da_eye_track = ds_box.sel(lat = ds_relative_track[-1].index.get_level_values('lat')[0],
                                      lon = ds_relative_track[-1].index.get_level_values('lon')[0], method = 'nearest')
...
def storm_tracker_mslp_ens(ds, df_ws, large_box, small_box, smooth_step = None):
    ...
    for numbers in ds.number:
        ...
        storm_tracker_list.append(storm_tracker_mslp_updated(ds.sel(number = numbers), df_ws, smooth_step = smooth_step,  large_box = large_box, small_box = small_box))
...
def storm_composite_ens(ds, df_center = None, radius = 3):
    ...
    for numbers in ds.number.values:  # convert numbers to a numpy array
        list_storm_composite.append(storm_composite(ds.sel(number = numbers), df_center = df_center[['lat','lon','number']].loc[df_center['number']== numbers.item()], radius = radius))  # convert numbers to a native Python int
...
def plot_minimum_track(df, df2=None):
    ...
    if 'number' in df.columns:
        sns.lineplot(data=df, x='lon', y='lat', hue='number', palette='tab10', linewidth=1.5, estimator=None, ax=ax,
                     transform=ccrs.PlateCarree())
        # hide legend
        ax.get_legend().remove()
    else:
        sns.lineplot(data=df, x='lon', y='lat', linewidth=1.5, estimator=None, ax=ax, transform=ccrs.PlateCarree())

    # Plot the second DataFrame if provided
    if df2 is not None:
        if 'number' in df2.columns:
            sns.lineplot(data=df2, x='lon', y='lat', hue='number', palette='tab10', linewidth=1.5, estimator=None, ax=ax,
                         transform=ccrs.PlateCarree(), linestyle='--')  # Using dashed line for the second track
        else:
            sns.lineplot(data=df2, x='lon', y='lat', linewidth=1.5, estimator=None, ax=ax, transform=ccrs.PlateCarree(),
                         linestyle='--')
...
def plot_cumulative_precipitation_timeseries(ds, lat, lon, ds_era5=None):
    ...
    sns.lineplot(data=df, x='time', y='Ptot_cumulative', hue='number', palette='tab10', linewidth=1.5, ax=ax, legend=False)
    ...
    ax.set_title('Cumulative Precipitation Time Series at location ({}, {})'.format(lat, lon))
    ax.set_xlabel('Time')
...
def plot_var_timeseries(ds, lat, lon, variable, ds_era5=None):
    ...
    sns.lineplot(data=df, x='time', y=variable, hue='number', palette='tab10', linewidth=1.5, ax=ax)
    ...
    if ds_era5 is not None:
        ...
        ax.plot(df_era5['time'], df_era5[variable], color='black', linestyle='dashed', linewidth=2.5)
...
def plot_comp_variable_timeseries(ds_composite, variable, ds_era5_composite=None, agg_method='mean', obs_data=None):
    ...
    if ds_era5_composite is not None:
        ...
        ax.plot(df_era5['time'], df_era5[variable], color='black', linestyle='dashed', linewidth=2.5, label = 'ERA5')
    ...
    if obs_data is not None:
        ...
        ax.plot(obs_data['time'], obs_data[variable], color='red', linestyle='dotted', linewidth=4, label = 'Observed', zorder = 10)
